...
function isAggregateScoreLabel(value) {
  const label = normalizeRubricText(value);
  return /(?:^|\s)(?:total|raw total|final score|overall score|ce penalty)(?:\s|$)/i.test(label)
    || /(?:সর্বমোট|মোট\s*নম্বর|মোট\s*স্কোর|কাঁচা\s*নম্বর|ফাইনাল\s*নম্বর|চূড়ান্ত\s*নম্বর|চূড়ান্ত\s*নম্বর)/i.test(label);
}
...
function parseMarkdownScoreRows(source, rubric) {
  ...
  for (const line of section.split('\n')) {
    ...
    if (isAggregateScoreLabel(label)) continue;
...
function fallbackQaScore(source, rubric) {
  const normalizedSource = toEnglishDigits(String(source || ''));
  const matches = [...normalizedSource.matchAll(/(?:চূড়ান্ত\s*স্কোর|ফাইনাল\s*স্কোর|সর্বমোট[^\n]*স্কোর|final\s*score|overall\s*score|total\s*score)[^\n]{0,120}?(\d+(?:\.\d+)?)\s*\/\s*(\d+(?:\.\d+)?)/gi)];
...
function parseQaMarkdownCall(source, fileName, rubric) {
  ...
  const ceEvidenceSource = visibleSource
    .replace(/\bCE\s*Detected\s*:\s*(?:false|no)\b/gi, '')
    .replace(/\bCE\s+(?:not\s+detected|was\s+not\s+detected|was\s+not\s+found)\b/gi, '')
    .replace(/(?:কোনো|কোনও)\s+(?:ক্রিটিক্যাল\s*এরর|CE)[^\n।]{0,80}(?:পাওয়া\s*যায়নি|পাওয়া\s*যায়নি|চিহ্নিত\s*হয়নি|চিহ্নিত\s*হয়নি|শনাক্ত\s*হয়নি|শনাক্ত\s*হয়নি)/gi, '');
  const visibleCe = /(?:🚨|Critical\s*Error\s+Detected|\bCE\s+(?:Detected|পাওয়া\s*গেছে|পাওয়া\s*গেছে|শনাক্ত|পরিলক্ষিত|কার্যকর))/i.test(ceEvidenceSource);
  ...
  const agentMatch = String(source || '').match(/\*\*(?:এজেন্টের নাম|এডভাইসরের নাম|অ্যাডভাইজরের নাম|Agent(?:'s)? Name|Advisor(?:'s)? Name)\s*:\*\*\s*([^\n]+)/i);
