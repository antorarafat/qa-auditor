...
function stringList(object, field) {
  if (!Array.isArray(object?.[field]) || !object[field].length) throw new Error(`AI response is missing ${field}.`);
  return object[field].map(value => String(value || '').trim()).filter(Boolean);
}
...
function validateQaResult(value, rubric) {
  ...
  const reconciled = rubric.rows.map(row => {
    ...
    const achieved = Number(score.achieved);
    if (!Number.isFinite(maximum) || Math.abs(maximum - row.maximum) > 0.001) throw new Error(`AI changed the maximum for ${row.parameter}.`);
    if (!Number.isFinite(achieved) || achieved < 0 || achieved > maximum) throw new Error(`AI returned an invalid achieved score for ${row.parameter}.`);
    ...
    return {
      ...
      achieved,
      deducted,
      timestamp: String(score.timestamp || '—').trim() || '—',
      deductionReason: String(score.deduction_reason || '').trim() || '—'
    };
  });
  ...
  return {
    ...
    deductionJustifications: stringList(value, 'deduction_justifications'),
    ...
    actionableTips: stringList(value, 'actionable_tips'),
...
function validateSummaryResult(value) {
  return {
    recurringIssues: stringList(value, 'recurring_issues'),
    bestAndWorstCalls: stringField(value, 'best_and_worst_calls'),
    overallRecommendations: stringList(value, 'overall_recommendations')
  };
}
...
function validateVoiceResult(value) {
  return {
    ...
    customerQuestions: stringList(value, 'customer_questions'), barriers: stringList(value, 'barriers'),
    productFeedback: stringList(value, 'product_feedback'), advisorName: stringField(value, 'advisor_name'),
    objectionHandlingAssessment: stringField(value, 'objection_handling_assessment'), improvementAreas: stringList(value, 'improvement_areas'),
    actionableRecommendations: stringList(value, 'actionable_recommendations')
  };
}

function validateCoachingResult(value) {
  const corrections = Array.isArray(value?.script_corrections) ? value.script_corrections : [];
  if (!corrections.length) throw new Error('AI response is missing script_corrections.');
  return {
    advisorName: stringField(value, 'advisor_name'), callTopic: stringField(value, 'call_topic'),
    salesPitchAudit: stringList(value, 'sales_pitch_audit'), tonePitchAnalysis: stringList(value, 'tone_pitch_analysis'),
    ...
    weeklyGrowthPlan: stringList(value, 'weekly_growth_plan')
  };
}
...
function scoreRowsMarkdown(scores) {
  return scores.map(row => `${row.category} — ${row.parameter} | ${row.maximum} | ${row.achieved} | ${row.deducted} | ${row.timestamp} | ${row.deductionReason}`).join('\n');
}

function scriptRowsMarkdown(items) {
  return items.length ? items.map(item => `${item.timestamp} | ${item.wrong} | ${item.correct}`).join('\n') : '— | — | —';
}
...
function aggregateQaResults(results, fileNames) {
  ...
  const agentRows = Array.from(byAgent, ([agent, calls]) => {
    ...
    return `${agent} | ${calls.length} | ${formatNumber(scores.reduce((a, b) => a + b, 0) / calls.length)} | ${formatNumber(Math.max(...scores))} | ${formatNumber(Math.min(...scores))} | ${calls.filter(call => call.ceDetected).length}`;
  }).join('\n');
  const categoryRows = Array.from(byCategory, ([category, value]) => `${category} | ${formatNumber(value.maximum / value.count)} | ${formatNumber(value.achieved / value.count)} | ${formatNumber(value.deducted / value.count)}`).join('\n');
...
function qaSchema(rubric) {
  return { type: 'object', additionalProperties: false, required: ['agent_name', 'call_summary', 'client_type_and_need', 'call_duration_and_tone', 'product_fact_check', 'ce_detected', 'ce_audit_details', 'ce_alert', 'scores', 'deduction_justifications', 'strengths', 'script_corrections', 'actionable_tips', 'overall_status'], properties: {
    agent_name: { type: 'string' }, call_summary: { type: 'string' }, client_type_and_need: { type: 'string' }, call_duration_and_tone: { type: 'string' }, product_fact_check: { type: 'string' }, ce_detected: { type: 'boolean' }, ce_audit_details: { type: 'string' }, ce_alert: { type: 'string' },
    scores: { type: 'array', minItems: rubric.rows.length, maxItems: rubric.rows.length, items: { type: 'object', additionalProperties: false, required: ['category', 'parameter', 'maximum', 'achieved', 'timestamp', 'deduction_reason'], properties: { category: { type: 'string' }, parameter: { type: 'string' }, maximum: { type: 'number' }, achieved: { type: 'number' }, timestamp: { type: 'string' }, deduction_reason: { type: 'string' } } } },
    deduction_justifications: { type: 'array', items: { type: 'string' } }, strengths: { type: 'array', items: { type: 'string' } },
    ...
    actionable_tips: { type: 'array', items: { type: 'string' } }, overall_status: { type: 'string' }
  } };
}
...
module.exports = { toEnglishDigits, parseQaRubric, loadTemplate, loadAndValidateTemplates, renderTemplate, validateQaResult, validateSummaryResult, validateVoiceResult, validateCoachingResult, renderQaCall, renderQaSummary, renderVoice, renderCoaching, auditResultRow, qaSchema, SUMMARY_SCHEMA, VOICE_SCHEMA, COACHING_SCHEMA };
