...
function validateQaResult(value, rubric) {
  ...
  const suppliedDeductions = stringList(value, 'deduction_justifications', true);
  const suppliedStrengths = stringList(value, 'strengths', true);
  const suppliedTips = stringList(value, 'actionable_tips', true);
  ...
  return {
    ...
    deductionJustifications: suppliedDeductions.length ? suppliedDeductions : (deductedRows.length ? deductedRows.map(row => `${row.parameter}: ${row.deductionReason}`) : ['কোনো নম্বর কাটা হয়নি।']),
    strengths: suppliedStrengths.length ? suppliedStrengths : (fullScoreRows.length ? fullScoreRows.map(row => `${row.parameter}: পূর্ণ নম্বর অর্জন করেছে।`) : ['—']),
    ...
    actionableTips: suppliedTips.length ? suppliedTips : (deductedRows.length ? deductedRows.map(row => `${row.parameter} উন্নত করতে নির্দিষ্ট অনুশীলন করুন।`) : ['বর্তমান মান বজায় রাখুন।']),
