...
function createMongoStore(config = {}) {
  ...
  async function uploadAudio(ownerEmail, file) {
    ...
    return new Promise((resolve, reject) => {
      const stream = bucket.openUploadStream(file.name, { metadata: { ownerEmail, mimeType: file.mimeType, sha256: file.sha256, size: buffer.length, durationSeconds: file.durationSeconds ?? null, createdAt: new Date() } });
      stream.once('error', reject);
      stream.once('finish', () => resolve({ fileId: stream.id, name: file.name, mimeType: file.mimeType, sha256: file.sha256, size: buffer.length, durationSeconds: file.durationSeconds ?? null }));
      stream.end(buffer);
    });
  }
  ...
  async function downloadAudio(file) {
    ...
    return { name: file.name, mimeType: file.mimeType, durationSeconds: file.durationSeconds ?? null, data: Buffer.concat(chunks).toString('base64') };
  }
  ...
  return {
    ...
    async listReportRecords(viewer, query = {}) {
      ...
      if (query.from || query.to) { filter.timestamp = {}; if (query.from) filter.timestamp.$gte = new Date(query.from); if (query.to) { const end = new Date(query.to); if (Number.isFinite(end.getTime()) && end.getUTCHours() === 0 && end.getUTCMinutes() === 0 && end.getUTCSeconds() === 0 && end.getUTCMilliseconds() === 0) end.setUTCHours(23, 59, 59, 999); filter.timestamp.$lte = end; } }
      ...
      return { items: page.map(item => ({ ...item, id: String(item._id), reportId: String(item.reportId), ownerUserId: item.ownerUserId ? String(item.ownerUserId) : null, _id: undefined })), nextCursor: hasMore ? String(page[page.length - 1]._id) : null, summary: { callsEvaluated: Number(summary?.callsEvaluated || 0), averageQaScore: summary?.averageQaScore != null && Number.isFinite(Number(summary.averageQaScore)) ? Number(Number(summary.averageQaScore).toFixed(2)) : null, ahtSeconds: summary?.ahtSeconds != null && Number.isFinite(Number(summary.ahtSeconds)) ? Number(Number(summary.ahtSeconds).toFixed(1)) : null, ceCount: Number(summary?.ceCount || 0) } };
    },
