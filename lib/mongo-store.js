...
function canonicalFieldCleanupUpdate(collectionName, document) {
  ...
  if (collectionName === 'user' && !has('normalizedUsername') && username) set.normalizedUsername = normalizeUsername(username);
...
function createMongoStore(config = {}) {
  ...
  async function ensureCollections(db) {
    ...
    for (const [name, validator] of [['sessions', sessionValidator], ['report_runs', reportValidator], ['product_brief', productValidator], ['product_category', categoryValidator], ['product_subcategory', subCategoryValidator], ['qa_scorecard', scorecardValidator], ['audio_evidence', evidenceValidator]]) await ensureCollection(db, name, validator);
