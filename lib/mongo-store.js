...
function createMongoStore(config = {}) {
  ...
  return {
    ...
    async updateNetworkAccess(networkAccess) { const db = await ready(); const value = { enabled: networkAccess?.enabled !== false, ipv4: [...new Set((networkAccess?.ipv4 || []).map(String))], ipv6: [...new Set((networkAccess?.ipv6 || []).map(String))] }; const now = new Date(); await db.collection('company').updateOne({ _id: 'primary' }, { $set: { networkAccess: value, updatedAt: now }, $setOnInsert: { createdAt: now } }, { upsert: true }); return value; },
