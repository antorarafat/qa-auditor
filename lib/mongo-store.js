...
function createMongoStore(config = {}) {
  ...
  async function companyName(db) {
    const company = await db.collection('company').findOne({}, { sort: { sourceRow: 1 } });
  ...
  return {
    ...
    async getAuditConfiguration() {
      ...
      const [products, parameters] = await Promise.all([
        db.collection('product_brief').find({}, { projection: { _id: 0, category: 1, subCategory: 1 } }).sort({ sourceRow: 1 }).toArray(),
        db.collection('qa_scorecard').find({}, { projection: { _id: 0, name: 1 } }).sort({ sourceRow: 1 }).toArray()
      ]);
    ...
    async getProductBriefs(categories = [], selections = []) {
      ...
      return db.collection('product_brief').find(filter, { projection: { _id: 0, category: 1, subCategory: 1, brief: 1 } }).sort({ sourceRow: 1 }).toArray();
    },
