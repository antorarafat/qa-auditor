...
function createMongoStore(config = {}) {
  ...
  async function ensureCollection(db, name, validator = {}) {
    ...
    if (!exists) await db.createCollection(name, { validator });
  ...
  async function ensureCollections(db) {
    ...
    for (const [name, validator] of [['sessions', sessionValidator], ['report_runs', reportValidator], ['product_brief', productValidator], ['qa_scorecard', scorecardValidator]]) await ensureCollection(db, name, validator);
  ...
  return {
    ...
    async listProductBriefs() { const db = await ready(); return (await db.collection('product_brief').find({}).sort({ category: 1, subCategory: 1 }).toArray()).map(item => ({ ...item, id: String(item._id), _id: undefined })); },
    async createProductBrief({ category, subCategory, brief }) { const db = await ready(); const now = new Date(); const result = await db.collection('product_brief').insertOne({ category, subCategory, brief, archived: false, version: 1, createdAt: now, updatedAt: now }); return { id: String(result.insertedId), category, subCategory, brief, archived: false, version: 1 }; },
    async updateProductBrief(id, fields) { const db = await ready(); const _id = objectId(id); if (!_id) throw new Error('Product brief was not found.'); const current = await db.collection('product_brief').findOne({ _id }); if (!current) throw new Error('Product brief was not found.'); await db.collection('product_brief').updateOne({ _id }, { $set: { ...fields, version: Number(current.version || 1) + 1, updatedAt: new Date() } }); return true; },
