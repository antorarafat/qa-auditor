...
function createMongoStore(config = {}) {
  ...
  function reportRecordFromItem(report, item, index, reportId) {
    ...
    return {
      ...
      agentName: item.agentName ? englishLabel(item.agentName) : '', durationSeconds: isCall && Number.isFinite(Number(item.durationSeconds)) ? Number(item.durationSeconds) : null,
  ...
  async function hydrateUser(db, user, includeSecrets = false) {
    ...
    const value = {
      ...
      name: String(user.username || email.split('@')[0]), role: user.role === 'admin' ? 'admin' : 'user',
  ...
  return {
    ...
    async createFirstAdmin({ email, username, password, initialCompanyName }) {
      ...
      try {
        ...
        await db.collection('company').insertOne({ _id: 'primary', companyName: String(initialCompanyName).trim(), createdAt: now, updatedAt: now });
    ...
    async updateCompany(name) { const db = await ready(); const value = String(name || '').trim(); const now = new Date(); await db.collection('company').updateOne({ _id: 'primary' }, { $set: { companyName: value, updatedAt: now }, $setOnInsert: { createdAt: now } }, { upsert: true }); return { companyName: value }; },
    ...
    async listReportRecordOptions(viewer) {
      const db = await ready(); const filter = viewer.role === 'admin' ? {} : { ownerUserId: objectId(viewer.id || viewer._id) };
      const [agents, processes, modes, parameters, owners] = await Promise.all([
        ...
        viewer.role === 'admin' ? db.collection('user').find({}, { projection: { _id: 1, username: 1, email: 1 } }).sort({ username: 1 }).toArray() : Promise.resolve([])
      ]);
    ...
    async listReportRecords(viewer, query = {}) {
      ...
      if (query.mode) filter.mode = String(query.mode).slice(0, 30);
      if (query.process) filter.process = String(query.process).slice(0, 200);
      if (query.parameter && !query.process) filter.process = String(query.parameter).slice(0, 200);
      if (query.agentName) filter.agentName = String(query.agentName).slice(0, 200);
      if (query.ce === 'true') filter.ce = true; else if (query.ce === 'false') filter.ce = { $ne: true };
      ...
      if (query.from || query.to) { filter.timestamp = {}; if (query.from) filter.timestamp.$gte = new Date(query.from); if (query.to) filter.timestamp.$lte = new Date(query.to); }
