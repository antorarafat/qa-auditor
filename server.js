...
function createApp(options = {}) {
  ...
  app.put('/api/admin/network-access', requireSameOrigin, requireAuth, requireReady, requireAdmin, jsonAdmin, async (req, res) => {
    const input = req.body?.networkAccess || req.body || {}; const ipv4Entries = Array.isArray(input.ipv4Entries) ? input.ipv4Entries : (Array.isArray(input.ipv4) ? input.ipv4.map(value => ({ value, selected: true })) : []); const ipv6Entries = Array.isArray(input.ipv6Entries) ? input.ipv6Entries : (Array.isArray(input.ipv6) ? input.ipv6.map(value => ({ value, selected: true })) : []); const ipv4 = [...new Set(ipv4Entries.filter(row => row?.selected !== false).map(row => String(row?.value || row?.address || '').trim()).filter(Boolean))]; const ipv6 = [...new Set(ipv6Entries.filter(row => row?.selected !== false).map(row => String(row?.value || row?.address || '').trim()).filter(Boolean))];
    const allIpv4 = ipv4Entries.map(row => String(row?.value || row?.address || '').trim()).filter(Boolean); const allIpv6 = ipv6Entries.map(row => String(row?.value || row?.address || '').trim()).filter(Boolean); if (allIpv4.some(rule => !/^\d{1,3}(?:\.\d{1,3}){3}(?:\/\d{1,2})?$/.test(rule) || !ipInCidr(rule.split('/')[0], rule)) || allIpv6.some(rule => !net.isIP(rule.split('/')[0]) || net.isIP(rule.split('/')[0]) !== 6 || (rule.includes('/') && (!Number.isInteger(Number(rule.split('/')[1])) || Number(rule.split('/')[1]) < 0 || Number(rule.split('/')[1]) > 128)))) return res.status(400).json({ error: 'Enter valid IPv4/IPv6 addresses or CIDR ranges.' });
    const proposed = { enabled: input.enabled !== false, ipv4, ipv6 }; if (proposed.enabled && !ipv4.length && !ipv6.length) return res.status(400).json({ error: 'Add at least one network address or enable all networks.' }); if (proposed.enabled && !networkIpAllowed(req.ip, proposed)) return res.status(400).json({ error: 'Keep your current network address allowed before saving this policy.' }); return res.json({ networkAccess: await dataStore.updateNetworkAccess(proposed) });
  });
  ...
  app.delete('/api/admin/product-categories/:id', requireSameOrigin, requireAuth, requireReady, requireAdmin, async (req, res) => { try { await dataStore.deleteProductCategory(req.params.id); return res.json({ ok: true }); } catch (error) { return res.status(400).json({ error: error.message }); } });
  ...
  app.delete('/api/admin/product-subcategories/:id', requireSameOrigin, requireAuth, requireReady, requireAdmin, async (req, res) => { try { await dataStore.deleteProductSubCategory(req.params.id); return res.json({ ok: true }); } catch (error) { return res.status(400).json({ error: error.message }); } });
  ...
  app.delete('/api/admin/product-briefs/:id', requireSameOrigin, requireAuth, requireReady, requireAdmin, async (req, res) => { try { await dataStore.deleteProductBrief(req.params.id); return res.json({ ok: true }); } catch (error) { return res.status(400).json({ error: error.message }); } });
