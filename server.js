...
function createProviderClient(config = {}) {
  ...
  async function callGemini(apiKey, audioFiles, prompt, schema) {
    const payload = { contents: [{ parts: [{ text: prompt }, ...audioFiles.map(file => ({ inlineData: { mimeType: file.mimeType, data: file.data } }))] }], generationConfig: { responseMimeType: 'application/json', responseSchema: schema } }; let lastError = 'No supported Gemini model responded.';
