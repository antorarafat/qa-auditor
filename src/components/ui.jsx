...
export function Card({ className = '', children, ...props }) { return <div className={cn('ui-card', className)} {...props}>{children}</div>; }
...
export function Switch({ checked, onChange }) { return <button type="button" className={cn('ui-switch', checked && 'checked')} role="switch" aria-checked={checked} onClick={() => onChange(!checked)}><span /></button>; }
